      ...
      {featuredArticle.metadata?.featured_image && (
        <div className="absolute inset-0">
          <img
            ...
            alt={featuredArticle.metadata?.headline || 'Featured article'}
        ...
        <div className="max-w-3xl">
          <div className="flex items-center space-x-4 text-white text-sm mb-4">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span>{formatDate(featuredArticle.metadata?.published_date || featuredArticle.created_at || new Date().toISOString())}</span>
            </div>
            ...
            <div className="flex items-center space-x-3">
              {featuredArticle.metadata?.author?.metadata?.profile_photo && (
                <img
                  ...
                  alt={featuredArticle.metadata?.author?.metadata?.full_name || 'Author'}
                  className="w-12 h-12 rounded-full"
                />
              )}
              <div>
                <p className="text-white font-medium">
                  {featuredArticle.metadata?.author?.metadata?.full_name || 'Anonymous'}
                </p>
                {featuredArticle.metadata?.author?.metadata?.job_title && featuredArticle.metadata?.author?.metadata?.company && (
