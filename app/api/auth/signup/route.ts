...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fix TypeScript errors by ensuring string types with proper null checks and fallbacks
    const userEmail = newUser.metadata?.email ? String(newUser.metadata.email) : email
    const userName = newUser.metadata?.full_name ? String(newUser.metadata.full_name) : fullName
